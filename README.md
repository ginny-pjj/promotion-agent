...
根目录放核心 `.py` 是 **有意保持扁平**，方便面试时快速打开讲解，不是结构错误。
...
| 实现思路 / AI 使用 / 遇到的问题 | [docs/交付说明.md](./docs/交付说明.md) |

### 面试 2 分钟演示建议

1. 启动网页，跑 **示例 A** → 指出「已转人工确认」  
2. （可选）跑 **示例 C** → 指出账号名不匹配  
3. 下载 Excel，展示「待人工确认」备注  
4. 打开 `human_review.py` 或 `validator.py`，说明：**规则发现问题，人工做最终决定**
