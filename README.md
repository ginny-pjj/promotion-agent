...
**方式一：直接看演示视频**（点击即可在 GitHub 页面内播放，无需安装任何环境）
...
浏览器打开 http://127.0.0.1:8000 ，粘贴 `demos/sample-A-*.txt` 两段文本后点击「开始处理」。

> 网页是**本机服务**（`127.0.0.1`），需要在本机启动后才能访问；只想快速看效果，请直接用上面的演示视频。
...
| 视频 | 内容 |
|---|---|
| [demo-A-笔数与日期异常.mp4](./demos/demo-A-笔数与日期异常.mp4) | 示例 A：垫付声明 7 笔、实际识别 5 条，且预计打款日期早于申请日期 → 转人工确认 |
| [demo-B-正常版本.mp4](./demos/demo-B-正常版本.mp4) | 示例 B：无异常，走常规抽查流程 |

> 点击即可在 GitHub 页面内播放，文件较大（约 90MB / 40MB），加载需要等待；也可先下载再离线观看。
...
| 抖加支付人 | 写入支付人 |
| 打款人 | 写入垫付打款人 |
| 打款日期 | 预留；「预计打款日期」不写入此列，避免与实付日期混淆，改为记入备注 |
| 备注（可选） | 异常账号写入「待人工确认」 |
